...
Kafka基本存储单元是分区。分区无法在多个broker之间进行再戏份，也无法在同一个broker的多个磁盘上进行再细分。 所以，分区的大小受到单个挂载点可用空间的限制
...
- broker会为分区里的每个片段打开一个文件句柄，哪怕片段是不活跃的。这样会导致打开过多的文件句柄，所以需要根据世纪情况调优
...
- 为了帮助broker能更快速地地味道制定的偏移量，Kafka为每个分区维护了一个索引
...
- 如果Kafka启动时候启用了清理功能，每个broker会启动一个清理管理器线程和多个清理县城，来负责执行清理任务。这些县城会选择污浊率较高的分区进行清理
- 清理县城痛殴内存里的一个map来清理
    - map里的每个元素包含了消息键的散列值和消息的偏移量，键的散列值说16B加上偏移量共24B
    - 清理一个1GB的日志片段，假设每个消息大小为1KB，那么这个片段包含一百万个消息，而我们只要用24MB的mao就可以清理这个片段
