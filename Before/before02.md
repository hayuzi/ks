...
三面：
- 自我介绍
- 如果让你设计一个延迟队列中间件，从Go语言的角度你准备怎么实现
- - 从基础的生产消费、ACK机制、内存、持久化数据存储、数据结构探讨，到高可用、分布式设计
- 这个过程中，让你去做优化，提高吞吐达到ms级别延时，你该怎么做
- - 可以参考Kafka的实现等等
- Go语言切片的底层结构，扩容的实现
- 两个切片一大一小、进行拷贝的话，性能会不一样吗
- MySQL数据库索引有哪些，索引的作用
- 对于千万量级的数据表，如昵称查询，你会使用什么索引结构
- 如果想要性能更好，性能上你还能做优化吗
- - 索引性能分析、分库分表、缓存等
...
三面：
- 很开放的聊自己的学习与从业经历
- 聊经历过的公司与项目，在各个阶段所扮演的角色，具体的人数，如何解决难题
- 做管理的时候，遇到员工自己的排期与你给的排期冲突的时候怎么处理。
- 聊自己如何学习入行、入行之后所做的提升
- 从数据结构、语言、组成原理聊到操作系统计算机网络知识
- - 讲了学习的一些书籍、并做了举例
- - 计算机组成原理、操作系统概念、数据结构、C语言、其他语言
- - 开涛 亿级流量网站架构、鸟哥的Linux私房菜、深入分布式缓存、深入浅出MqSQL等等
- - 讲业务架构设计与计算机底层设计的共通性、提到多级缓存的设计、分治思想，分布式运用
- 学习路径、以及如果现在重来一次，学习路径会不会有其他的改变
- - 描述了自己的路径像是 从一个球的表面深入到圆心，
- - 等应用层面工具的学习，web开发的外层深入底层数据内层
- 提一些问题
- 整体的感受，就是在聊天的过程中，掌握你的知识面以及知识深度, 少量提问，自主叙述
